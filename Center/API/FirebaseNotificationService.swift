...
//  Center
//
//  Created by Jidong Zheng on 10/1/25.
//
...
class FirebaseNotificationService: NSObject, MessagingDelegate, UNUserNotificationCenterDelegate {
    ...
    // MARK: - Error Logger
    let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "com.cornellappdev.center", category: #file)
            ...
            case .notDetermined:
                UNUserNotificationCenter.current().requestAuthorization(options: options) { granted, error in
                    if let error = error {
                        ...
                        self.logger.log("Notications-permission denied.")
                    }
                }
    ...
    func userNotificationCenter(
        ...
        // TODO: Customize based on app's need
    ...
    private func navigateToScreen(with navigationId: String) {
        ...
        logger.info("Navigating to screen with ID: \(navigationId)")
    }
}
